...
class FilterTable extends React.Component{
    ...
    handlePMaximumChange(e){
        let value = Math.max(+e.target.value, this.state.pMin + 1);
        if(value > this.state.pMax){
    ...
    handleRDMaximumChange(e){
        let value = Math.max(+e.target.value, this.state.rdMin + 1);
        if(value > this.state.rdMax){
            value = this.state.rdMax;
        }
        this.setState({
            rdMax: value,
        });
    ...
    handleCCMaximumChange(e){
        let value = Math.max(+e.target.value, this.state.ccMin + 1);
        if(value > this.state.ccMax){
            value = this.state.ccMax;
        }
        this.setState({
            ccMax: value,
        });
